...
# UPDATED: Import you custom SimpleRNN implementation
from Saghar.custom_simplernn import SimpleRNN  # Import your custom SimpleRNN from try1.py
...
model = Sequential([
    ...
    SimpleRNN(128, activation='tanh', return_sequences=True),  # First custom SimpleRNN layer with return_sequences=True
    BatchNormalization(),  # Apply Batch Normalization
    #Dropout(0.05),  # Apply Dropout (commented out as before)

    # CHANGED: Using custom SimpleRNN instead of keras.layers.SimpleRNN  
    SimpleRNN(128, activation='tanh'),  # Second custom SimpleRNN layer with 128 units and 'tanh' activation
    BatchNormalization(),  # Apply Batch Normalization
    #Dropout(0.05),  # Apply Dropout (commented out as before)
...
model.save("SimpleRNN_model_1.h5")
...
plt.savefig('confusion_matrix_1.png')  # Save Confusion Matrix plot (For server use)
...
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.savefig('accuracy_plot_1.png')  # Save accuracy plot (For server use)
...
def generate_test_vectors():
    ...
    with open('simplernn_test_vectors_1.v', 'w') as f:
